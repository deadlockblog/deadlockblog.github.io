...
Clicker. Maquina medium de HTB, web, PHP. De esas que miras la ficha y piensas "esto lo saco en una hora". Spoiler: no fue una hora. Pero lo que encontre dentro merece un post, porque el fallo es tan tonto y tan comun que da rabia.

La app es un juego de clicker. Si, un juego de hacer clicks. Tienes tu perfil de jugador con `username`, `nickname`, `password`, `role`, `clicks` y `level` en MySQL. Lo primero que me llamo la atencion fue la columna `role`. Los jugadores normales tienen `User`, y el objetivo es obvio: llegar a `Admin`. Pero no hay boton magico en la interfaz que te deje cambiar eso. Asi que toca abrir el capo y mirar que hay dentro.

# Leyendo el codigo: cuando el developer se lo pone facil

Abro `save_game.php`, el archivo que se ejecuta cuando guardas tu partida, y me encuentro con esto:
...
A ver si lo desgranamos. El codigo recorre *todos* los parametros que le llegan por GET y los mete en un array. Todos. Sin excepcion. Lo unico que hace es comprobar que el nombre del parametro no sea exactamente `role`. Si no lo es, pasa. Si lo es, te suelta un "Malicious activity detected!" y te echa. Todo lo demas, bienvenido sea. Y luego se lo pasa enterito a `save_profile()`.

Salto a `db_utils.php` para ver que hace esa funcion, y claro:
...
Aqui lo vi todo claro. La funcion construye el `SET` de la query SQL dinamicamente con *todo* lo que le pases. Sin allowlist de campos. Sin validacion de nombres. Sin nada. Si le mandas `clicks=100&level=5`, genera `SET clicks='100', level='5'`. Perfecto. Pero si le mandas `clicks=100&level=5&role=Admin`... pues genera `SET clicks='100', level='5', role='Admin'`. Asi, sin despeinarse. Sin preguntar. Sin pestanear.

Esto tiene un nombre: **Mass Assignment**. Y si no te suena o lo has visto de pasada sin darle importancia, quedate, porque es de esas vulnerabilidades que te encuentras cada dos por tres en engagements reales.
...
Ya tenemos claro que hay **Mass Assignment**. El vector obvio seria meter `&role=Admin` en la URL y ver que pasa. Pero el developer no era *completamente* tonto. Puso un filtro:
...
Si el parametro se llama `role`, te bloquea. Da igual que lo pongas en mayusculas, minusculas o mezcla, porque usa `strtolower()`. Primer intento al traste.

Pero aqui me quede un rato pensando. El filtro compara strings. Comparacion estricta con `===`. Si la string no es *exactamente* `"role"`, pasa. Entonces la pregunta se hace sola: que pasa si le meto un caracter que PHP vea como parte del nombre del parametro pero que MySQL ignore a la hora de ejecutar la query?
...
Ahora viene lo bonito. Cuando `save_profile()` mete ese nombre de parametro en la query SQL, genera esto:
...
Ese salto de linea entre `role` y `=` para MySQL es whitespace. Como un espacio. Le da absolutamente igual. La query es valida y se ejecuta sin problema. Role actualizado a `Admin`. Respuesta del servidor: `302 Found`, `Location: /index.php?msg=Game has been saved!`.
...
Con el tab es la misma historia:
...
Y el comentario SQL vacio es otra via completamente distinta:
...
PHP recibe `"role/**/"` como nombre de parametro. El filtro no lo reconoce. Y cuando llega a MySQL, el parser elimina el comentario `/**/` y queda `role = 'Admin'`. Limpio.
...
El `role` en la base de datos ya era `Admin`, pero `$_SESSION['ROLE']` seguia siendo `User`. Y claro, tiene sentido: la sesion de PHP se carga con los datos del usuario en el momento del login, no se refresca en cada request. El check de `admin.php` comprueba la sesion, no la base de datos:
...
Lo que deberia haber hecho el developer es definir una allowlist de campos aceptados:
